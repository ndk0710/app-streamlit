...
if __name__ == '__main__':
    ...
    for index, ym in enumerate(target_ym):
        ...
        for location in locations:
            ...
            # レイクハウス用のテーブル2（配台算出）
            answer2=[]
            for i, (m, p) in enumerate(zip(machines, products)):
                h50, process = p.split('_')
                for j in range(len(products)):
                    answer2.append([ym, location, m, h50, process,result_matrix[i][j]])

            answer2 = [row for row in answer2 if row[-1] != 0]

            # データ格納
            arrangement_output.extend(answer2)
