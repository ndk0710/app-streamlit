...
def find_matching_indices(list1, list2, check_value1, check_value2):
    for index, (value1, value2) in enumerate(zip(list1, list2)):
        if int(value1) == check_value1 and int(value2) == check_value2:
            return index
...
class Mathmatical:
    def __init__(self, products, machines, specs, mounts, number_of_units_owned):
        ...
        self.decision_variable = [f"{self.products[i]}__{self.machines[j]}" for i in range(len(self.specs))
                                  for j in range(len(self.specs[i]))
                                  if self.specs[i][j] > 0]
        self.last_index = 0
    ...
    def constraint_mount_definition(self):
        ...
        self.j_idx, self.i_idx = np.nonzero(transposed_specs_array)

        coeff_values = -transposed_specs_array[self.j_idx,self.i_idx]
        self.num_vars = coeff_values.shape[0]

        
        #ゼロで初期化
        A_ub = np.zeros((products_nums, self.num_vars), dtype=np.float32)
        A_ub[self.i_idx, np.arange(self.num_vars)] = coeff_values
        self.A_ub = A_ub
    ...
    def constraint_units_owned_definition(self):
        ...
        constraints = [
            [l for l in range(P) if self.transposed_specs[k][l]>0]
            for k in range(M)
        ]
        ...
        for item in condition_list:
            if item not in unique_list and len(item[1])>0:
        ...
        for machine_index, cons_indices in unique_list:
            ...
            for product_idx in cons_indices:
                idx=find_matching_indices(self.j_idx, self.i_idx, machine_index, product_idx)
                outputs[idx] = 1
    ...
    def solve(self):
        ...
        if result.success:
            ...
            #CSV出力
            csv_output = [[0 for _ in range(self.products_nums)]for _ in range(self.num_vars)]
            self.number_of_calc_detail = [0 for _ in range(self.products_nums*self.num_vars)]
            self.row_names = []
            self.column_names = []
            for i in range(self.num_vars):
                # 特定の文字列が存在する位置を検索
                row_name = self.decision_variable[i]
                row_index = row_name.find('__')
                self.row_names.append(row_name[row_index+2:])
                col_index = row_name.find('_')
                self.column_names.append(row_name[:col_index])
                for j in range(self.products_nums):
                    if self.i_idx[i] == j:
                        csv_output[i][j] = result.x[i]
                        #計算結果をメンバ変数に格納（小数点以下3桁目で四捨五入）
                        self.number_of_calc_detail[self.products_nums*i+j] = (round(result.x[i],2))

            self.column_names = list(dict.fromkeys(self.column_names))        
            ...
            #計算結果取得
            self.matrix = [self.number_of_calc_detail[i*self.products_nums:(i+1)*self.products_nums] for i in range(self.num_vars)]
            tmp =[]

            j_idx_value = list(dict.fromkeys(self.j_idx))
            
            for j_value in j_idx_value:
                # 特定の文字列を含む要素のインデックスを抽出
                indices = [index for index, value in enumerate(self.j_idx) if value==j_value]
                
                # インデックスに該当する行を抽出して足す
                result = [0] * len(self.matrix[0])
                for index in indices:
                    result = [x + y for x, y in zip(result, self.matrix[index])]
                tmp.append(result)
                check=0
            oo=0
            self.matrix = tmp
