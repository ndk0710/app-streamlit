...
def update_machines_number_of_units_owned(result_matrix, number_of_units_owned, machines):
    ...
    for index_to_remove in indexs_to_remove:
        del number_of_units_owned[index_to_remove]
        del machines[index_to_remove]
...
if __name__ == '__main__':
    ...
    mounts_info = pd.read_csv("./csv/生産量_25F1-26F1.csv", encoding='utf-8')
    ...
    for mounts in production_planning:
        ...
        if flag == False:
            ...
            #メンバ変数（設備/設備生産能力）
            machines.extend(['SCF-28（DLW32共用）','SCF-29/FMAP-1*（DLW32共用）'])
            machines=remove_duplicates_keep_first(machines)
            specs = get_specs(products, machines, specs_info)
