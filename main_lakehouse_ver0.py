...
from lib.cost_mathmatical_optimization_fabric_ver0 import Cost_Mathmatical
import pandas as pd
import copy
import math
...
if __name__ == '__main__':
    ...
    for index, ym in enumerate(target_ym):
        ...
        #拠点間用の変数
        total_short_machines, total_machine, total_product, total_matrix = [], [], [], []
        ...
        for location in locations:
            ...
            total_product.extend(products)

            # 合計マトリクスの更新
            if not total_matrix:
                total_matrix.append(result_matrix)
            else:
                total_matrix = [
                    row1 + row2
                    for row1, row2 in zip(total_matrix[0], result_matrix)
                ]
        ...
        if has_surplus:
            ...
            #コスト数理最適化のインスタンス作成
            cost_mount_info = [[row[1], row[2], (-row[3] if row[3] < 0 else 0)] for row in shoratge_output if row[0] == ym]
            cost_mount_info = pd.DataFrame(cost_mount_info, columns=['拠点', '設備', '保有台数'])
            cost_math_matical = Cost_Mathmatical(total_product, cost_info, cost_mount_info, total_short_mount_info, total_matrix)
